function dydt = SOX2_ISL1_autoreg(t,y,tvec,SMAD4,opts)
%t = current time
...
%%%parameters%%%
%production rates
betas = opts.betas; lambdai = opts.lambdai; betaa = opts.betaa;
%degradation rates
alphas = opts.alphas; alphai = opts.alphai;
%hill function thresholds
Ksi = opts.Ksi; Kis = opts.Kis; Ka = opts.Ka;
%SMAD4/SOX2 slope
lambda = opts.lambda;
...
ns = opts.ns;
%hill coefficient for SOX2 positive autoregulation
na = opts.na;

dydt = NaN(size(y));
%sox2:
dydt(1) = (betas - lambda*smad4 + betaa*y(1)^na/(Ka^na + y(1)^na))/(1 + (y(2)/Kis)^ns) - alphas*y(1);
%isl1:
