...
conds = unique(stats.sample{condi})';
for si = conds
...
ratio_profile_colstd = imfilter(std(cat(3, ratio_profiles{:}),[],3),[1 1 1 1 1]'/5, 'replicate');
    ...
    for cii = numel(options.nucChannels):-1:1
        ...
        if options.std
            ...
            elseif strcmp(options.stdMethod,'neighborCells')
                errorbar(r, nuc_profile(:,ci), nuc_profile_std(:,ci),'LineWidth',1, 'Color', options.colors(cii,:));
            end
        end
    end
    if options.normalize
...
if ~isempty(options.cytChannels)
    for cii = numel(options.cytChannels):-1:1
        ...
        plot(r, cyt_profile(:,ci),'LineWidth',3, 'Color', options.colors(cii,:))
    ...
    for cii = numel(options.cytChannels):-1:1
        ...
        if options.std
            if strcmp(options.stdMethod,'perColony')
                errorbar(r, cyt_profile(:,ci), cyt_profile_colstd(:,ci),'LineWidth',1, 'Color', options.colors(cii,:));
            elseif strcmp(options.stdMethod,'neighborCells')
                errorbar(r, cyt_profile(:,ci), cyt_profile_std(:,ci),'LineWidth',1, 'Color', options.colors(cii,:));
            end
        end
    end
    if options.normalize
...
if ~isempty(options.ratioChannels)
    for cii = numel(options.ratioChannels):-1:1
        ...
        plot(r, ratio_profile(:,ci),'LineWidth',3, 'Color', options.colors(cii,:))
    ...
    for cii = numel(options.ratioChannels):-1:1
        ...
        if options.std
            if strcmp(options.stdMethod,'perColony')
                errorbar(r, ratio_profile(:,ci), ratio_profile_colstd(:,ci),'LineWidth',1, 'Color', options.colors(cii,:));
            elseif strcmp(options.stdMethod,'neighborCells')
                errorbar(r, ratio_profile(:,ci), ratio_profile_std(:,ci),'LineWidth',1, 'Color', options.colors(cii,:));
            end
        end
    end
    if options.normalize
...
res = struct('r',r, 'nuc_profile',nuc_profile, 'nuc_profile_std',nuc_profile_std,...
    ...
    'cyt_profile_colstd',cyt_profile_colstd,'trueradius',trueradius);
end
