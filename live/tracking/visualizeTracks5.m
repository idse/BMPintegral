function visualizeTracks5(positions, opts)
...
if ~opts.use_tif
    if isfield(positions.cellData,'area')
        areas = transpose({positions.cellData.area});
    else
        areas = transpose({positions.cellData.nucArea});
    end
end
...
%         fname = fullfile(path, strcat(name,'.tif'));
        name =  [positions.filename,'000',num2str(nucChannel-1)];
%         fname = fullfile(opts.dataDir,[name,'.tif']);
        path = opts.dataDir;
        testim = loadImage(positions,path,nucChannel-1,1);
...
%         disp(strcat(num2str(round(j/ntimepoints*100)),"%"))
    ...
    if opts.use_tif
        if opts.default_tif
            ...
            %read the image at frame j, if using a tif
            frame = im2double(imadjust(imread(fname,j)));
        end
    else
