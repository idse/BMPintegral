function [newedges, badedges, costs] = graphmergelinker_v2(G,...
...
mergepoints = find(Din == 2); %nodes with two edges in
...
for ii = 1:length(mergepoints)
    ...
    %get position, size info for output nodes
...
%         nodes = [nodes; outs];
        %instead, compare position, size of two inputs, choose the one that
        %more closely matches the output*********
        c = pdist2(xyin,xyout,'squaredeuclidean').*(1 +...
            2*abs(Ain - Aout')./(Ain + Aout'));
        if c(1) < c(2)
            newbadedges = [ins(2) outs];
        else
            newbadedges = [ins(1) outs];
        end
        badedges = [badedges; newbadedges];
    end
end

%%%%% iterate over splits for which Divs is false and that are not included
%%%%% in the badedges/newedges lists above and compare the output nodes to
%%%%% the start node to determine which edge to remove
splitpoints = find(Dout == 2 & ~allDiv);
splitpoints = splitpoints(~ismember(splitpoints,badedges(:)));
for ii = 1:length(splitpoints)
    ins = splitpoints(ii);
    outs = successors(G,ins);
    xyin = allpoints(ins,:);
    xyout = allpoints(outs,:);
    Ain = allA(ins); Aout = allA(outs);
    c = pdist2(xyin,xyout,'squaredeuclidean').*(1 +...
        2*abs(Ain - Aout')./(Ain + Aout'));
    if c(1) < c(2)
        newbadedges = [ins outs(2)];
    else
        newbadedges = [ins outs(1)];
    end
    badedges = [badedges; newbadedges];
end
